...
A hyper-local, community-optimized food delivery engine designed for high-density living. BlocBites intelligently clusters neighborhood orders, dynamically adapts delivery zones to live weather, and enables multi-payer shared carts—secured by a robust JWT and OAuth2 architecture.

Built with Spring Boot 4, Spring Security, PostgreSQL, Kafka, WebSocket/STOMP, and Razorpay.
...
BlocBites solves this by treating the backend as a spatially aware, dynamic ecosystem. It automates batching, strictly manages real-world availability, and secures the entire pipeline from group-cart to gateway.
